...
public class Parser {
    ...
    public Parser(Scanner scanner) {
        parsingTable = new ParsingTable(GrammarRule.ALL_GRAMMAR_RULEs);
    ...
    public void parse() {
        ...
        while (true) {
            ...
            if (symbolStack.empty() && !scanner.hasNextToken()) { // no more input
                ...
                GrammarRule grammarRuleToExpandNonTerminal = parsingTable.getParsingRule(nonTerminal, currentToken.getTokenType());

                if (grammarRuleToExpandNonTerminal == null) {
                    grammarRuleToExpandNonTerminal = correctNtErrorIfEnabled(nonTerminal);
                }

                symbolStack.pop();

                pushExpansionToSymbolStack(grammarRuleToExpandNonTerminal);
            }
    ...
    private GrammarRule correctNtErrorIfEnabled(NonTerminal nonTerminal) {
        didThrowError = true;

        List<TokenType> expectedTokens = parsingTable.getAugmentedFirstSet(nonTerminal);

        GrammarRule grammarRuleToExpandNonTerminal = null;

        if (errorCorrection) {
            NonTerminal newFocus = correctError(nonTerminal);
            grammarRuleToExpandNonTerminal = parsingTable.getParsingRule(newFocus, currentToken.getTokenType());

        } else {
            throwError(scanner.getLexicalError(currentToken, expectedTokens));
        }

        return grammarRuleToExpandNonTerminal;
    }

    private void pushExpansionToSymbolStack(GrammarRule grammarRuleToExpandNonTerminal) {
        GrammarSymbol[] expansion = grammarRuleToExpandNonTerminal.getExpansion();
